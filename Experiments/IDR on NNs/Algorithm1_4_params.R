...
params=matrix(nrow=nsplits,ncol=12)
...
for (split in 1:nsplits){
  ...
  for (j in 1:nrow(grid2)){
    ...
    nu_init2=append(nu_init2,as.numeric(grid4[which.min(of_c_h_init),])[1])#CHANGE
  ...
  nu_init2_opt=nu_init2[which.min(MSE)]
...
#              '2nd nu:',nu2_opt,"| ",
 ...
 #             
  ...
  params[split,]=c(lr_opt,bs_opt,c_opt,nu_opt_local,nu_opt_gs, nu_opt,
                   h_opt_gs, h_opt,nu2_opt,nu_init2_opt, h_opt_init,c_opt_2)
